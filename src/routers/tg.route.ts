...
function getTgRoute() {
	...
	logger.info(`Get Tg Route ${env.TG_BOT_TOKEN?.length}`);
	if (!env.TG_BOT_TOKEN) {
		logger.debug(
			"TG_BOT_TOKEN is not set, skipping Telegram bot initialization",
		);
		return {
			bot: null,
			tgRoute,
	...
	//TODO: after you get domain - setup webhook in production
	...
	bot.command("start", async (ctx) => {
		...
		try {
			...
			logger.error(
				{ err, userId: from.id },
				"Failed to register user on /start",
			);
		}
...
export const { bot, tgRoute } = getTgRoute();
